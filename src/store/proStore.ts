 ...
 * Pro Store
 *
 * Manages the Pro subscription state and streak shield offer flow.
 *
  ...
  load: async () => {
    try {
      ...
      if (stored) {
        ...
        // Legacy trial cleanup: remove any stored trial state and disable Pro
        // if it was only granted by the old gift trial.
        const hasLegacyTrial = Boolean(data.trialExpiresAt);

        const newState: ProStoreState = {
          isPro: hasLegacyTrial ? false : (data.isPro ?? false),
        ...
        set(newState);
        if (hasLegacyTrial) await persist(newState);
      } else {

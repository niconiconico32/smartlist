...
export default function SuccessChartSlide({ onNext }: any) {
  ...
  const opacity1 = useAnimatedProps(() => ({
    opacity: progress.value > 0.05 ? withTiming(1) : 0,
  }));
  const opacity2 = useAnimatedProps(() => ({
    opacity: progress.value > 0.45 ? withTiming(1) : 0,
  }));
  const opacity3 = useAnimatedProps(() => ({
    opacity: progress.value > 0.95 ? withTiming(1) : 0,
  }));
